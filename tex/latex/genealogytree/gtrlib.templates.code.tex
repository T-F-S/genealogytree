...
\NewDocumentCommand \gtr@template@medical@interpreter { +m }
  {
    \long\def\gtrBoxContent{}
    \tl_if_blank:nF { #1 }
      {
        \box_clear_new:N \l__getree_template_medical_box
        \hbox_set:Nn \l__getree_template_medical_box
          {
            \kvtcb@fontupper
            \begin{ tabular } { @ {} c @ {} }
            #1
        ...
        \exp_args:Ne \gtrset
          {
            distance = \dim_eval:n { \box_wd:N \l__getree_template_medical_box +2.5mm }
          }
        \gtrset
          {
            box =
              {
                underlay =
                  {
                    \node [ left,tcbcolupper,outer~sep=0mm,inner~sep=0.5mm ] at (frame.west)
                      {
                        \box_use_drop:N \l__getree_template_medical_box
                      } ;
                  }
              },
          }
...
\gtrset{
  template/medical/.style={
    ...
    box={%
      ...
      colback=white,
      colframe=black,
      colupper=gtr_Bright_Red,
      fontupper=\sffamily\footnotesize,
    },
    edges={foreground=black,background=white},
    male/.style={box={arc is curved,arc=1mm,colframe=gtr_Male_Frame,colback=gtr_Male_Background}},
    female/.style={box={arc is curved,circular arc,colframe=gtr_Female_Frame,colback=gtr_Female_Background}},
    neuter/.style={box={arc is angular,arc=3.5mm,colframe=gtr_Neuter_Frame,colback=gtr_Neuter_Background}},
    carrier/.style={box={%
      underlay={%
        \begin{tcbclipframe}
          \path [fill=tcbcolframe] (frame.south west) rectangle (frame.north);
        \end{tcbclipframe}%
      }}},
    affected/.style={box={colback=tcbcolframe}},
    deceased/.style={box={%
      underlay={%
        \path[draw=tcbcolframe,line width=\kvtcb@right@rule]
          ([xshift=-0.5mm,yshift=-0.5mm]frame.south west) -- ([xshift=0.5mm,yshift=0.5mm]frame.north east);
      }}},
    content interpreter=\gtr@template@medical@interpreter,
  },
}
